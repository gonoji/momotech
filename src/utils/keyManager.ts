...
export class KeyManager {
    ...
    public static isDown(key: string){
        return this.getButton(key).isDown;
    }public static isUp(key: string){
        return this.getButton(key).isUp;
    }
    public static replaceKey(key1: string, key2: string){
        if(this.keys.has(key1) && this.keys.has(key2)){
            const key01 = this.getButton(key1);
            const key02 = this.getButton(key2);
    ...
    public static getButton(key: string): Phaser.Input.Keyboard.Key{
        if(!this.keys.has(key)) throw new Error('指定されたキーが見つかりません : ' + key);
